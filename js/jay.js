let Jay_cookiesTime = null;
let Jay_musicPlaying = false;
let Jay_keyboard = false;
let Jay_intype = false;
// 私有函数
var Jay = {
    // 检测显示模式
    ...
    is_Post: function() {
        ...
        if (url.indexOf("/posts/") >= 0) { //判断url地址中是否包含code字符串
            return true;
        } else {
            return false;
        }
    ...
    addFriendLinksInFooter: function() {
        var fetchUrl = "https://fcircle.lovelu.top/randomfriend?num=3"
        fetch(fetchUrl)
            .then(res => res.json())
            .then(json => {
    ...
    addTag: function() {
        //添加new标签
        if (document.querySelector('.Jay-tag-new')) {
            $(".Jay-tag-new").append(`<sup class="Jay-tag Jay-tag-new-view">N</sup>`)
        }
        //添加hot标签
        if (document.querySelector('.Jay-tag-hot')) {
            $(".Jay-tag-hot").append(`<sup class="Jay-tag Jay-tag-hot-view">H</sup>`)
        }
    },
    ...
    initThemeColor: function() {
        if (Jay.is_Post()) {
            ...
            if (currentTop === 0) {
                let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--Jay-main');
                Jay.changeThemeColor(themeColor);
            } else {
                let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--Jay-background');
                Jay.changeThemeColor(themeColor);
            }
        } else {
            let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--Jay-background');
    ...
    showLoading: function() {
        ...
        let cardColor = getComputedStyle(document.documentElement).getPropertyValue('--Jay-card-bg');
        Jay.changeThemeColor(cardColor);
    },
    ...
    musicToggle: function() {
        ...
        if (Jay_musicPlaying) {
            ...
            Jay_musicPlaying = false;
        } else {
            document.querySelector("#nav-music").classList.add("playing");
            document.getElementById("menu-music-toggle").innerHTML = msgPause;
            document.querySelector("#consoleMusic").classList.add("on");
            Jay_musicPlaying = true;
        }
