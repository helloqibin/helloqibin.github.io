var fdata = {
  ...
  apiurl: 'https://fcircle.lovelu.top/',
  defaultFish: 100,
  hungryFish: 100,
}
...
function fetchRandomPost () {
  ...
  if (randomPostWorking === false) {
    ...
    // $(".random-post-start").css("transition-duration",randomTime / 1000 + 's');
    // $(".random-post-start").css("transform","rotate("+randomRotate+"deg)");
    $(".random-post-start").css("opacity", "0.2");


    //判断是否饥饿
    if (((randomPostClick * fdata.hungryFish + fdata.defaultFish) < randomPostTimes) && (Math.round(Math.random()) == 0)) {
      document.getElementById("random-post").innerHTML = "因为只钓鱼不吃鱼，过分饥饿导致本次钓鱼失败...(点击任意一篇钓鱼获得的文章即可恢复）"
        ...
        .then(json => {
          ...
          if (document.querySelector('#random-post')) {
            window.setTimeout(function () {
              ...
              $(".random-post-start").css("opacity", "1");
            }, randomTime)
          }
        })
      randomPostWorking = false;
...
function initRandomPost () {
  ...
  if (localStorage.randomPostTimes) {
    ...
    $(".random-post-start").css("transition-duration", '0.3s');
    $(".random-post-start").css("transform", "rotate(" + randomPostTimes * 360 + "deg)");
  }
  fetchRandomPost();
}
